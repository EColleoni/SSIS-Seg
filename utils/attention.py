...
class SelfAttnModel(tf.keras.Model):
    ...
    def __init__(self, input_dims, data_format='channels_last', **kwargs):
        ...
        self.value_conv = tf.keras.layers.Conv2D(filters=input_dims,
                                        ...
                                        strides=2,
                                        data_format=data_format)
        self.conv_transpose = tf.keras.layers.Conv2DTranspose(filters=input_dims,
                                        kernel_size=2,
                                        strides=2,
                                        data_format=data_format)
    ...
    def call(self, inputs, training=False):
        ...
        return self.attn([q, k, v, inputs], self.conv_transpose, self.input_dims)
...
class _Attention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, conv_tr, input_dims):
        if len(inputs) != 4:
        ...
        if self.data_format == 'channels_first':
            ...
            out = tf.reshape(
                tf.transpose(out, (0, 2, 1)), (batchsize, height, width, input_dims))
